...
class User extends Model {
    ...
    public function loginLost($email){
        ...
        $message = 'Vous avez effectu&eacute; une demande de r&eacute;cup&eacute;ration de votre identifiant.
            '."\n".'Votre adresse e-amil est li&eacute;e à l\'login : '.$user['login'];
        if(!mail($user['email'], 'R&eacute;cup&eacute;ration d\'login', $message)){
    ...
    public function passwordLost($login){
        ...
        $message = 'Vous avez effectu&eacute; une demande de r&eacute;cup&eacute;ration de mot de passe.
            '."\n".'Veuillez cliquer sur le lien suivant pour le r&eacute;initialiser : http://mon-site/'.$url;
        if(!mail($user['email'], 'R&eacute;cup&eacute;ration de mot de passe', $message)){
            return array('in_error' => true, 'errors' => array('Impossible d\'envoyer le mail'));
        }
    ...
    private function incrementLostAttempts($user){
        if($user['lost_attempts'] <= 2){
            ...
            foreach($config['admins'] as $admin_mail){
                mail($admin_mail, 'Compte bloqué', $message);
            }
            return false;
        }
    }
